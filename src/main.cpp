...
TestProcess::TestProcess() : PProcess("Command line VoIP testphone", "sipcmd2")
{
}

void TestProcess::Main()
{
    std::cout << "Starting sipcmd2" << std::endl;
...
bool Manager::Init(PArgList &args)
{
    ...
    if (!protocol.compare("sip"))
    {
        ...
        if (args.HasOption('c'))
        {
            ...
            login_sip = args.GetOptionString('u');
            password_sip = args.GetOptionString('c');
                                ...
                                OpalMediaStream &stream)
{
    ...
    std::cout << (stream.IsSink() ? "streaming media to " : "recording media from ")
              << prefix << std::endl;
